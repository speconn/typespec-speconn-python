...
function emitPython(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    client.push("from speconn import SpeconnClient, HttpClient");
    client.push(`from ${importBase} import *\n`);
    client.push(`class ${svc.serviceName}Client(SpeconnClient):`);
    client.push(`    def __init__(self, base_url: str, http_client: HttpClient | None = None) -> None:`);
    client.push(`        super().__init__(base_url, http_client)`);
    for (const rpc of svc.rpcs) {
      const inputName = rpc.inputType?.name || "dict";
      const outputName = rpc.outputType?.name || "dict";
      if (rpc.isStream) {
        client.push(`    async def ${rpc.name}(self, req: ${inputName}, *, headers: dict[str, str] | None = None) -> list[${outputName}]:`);
        client.push(`        return await self.stream("${rpc.path}", req, ${outputName}, headers=headers)`);
      } else {
        client.push(`    async def ${rpc.name}(self, req: ${inputName}, *, headers: dict[str, str] | None = None) -> ${outputName}:`);
        client.push(`        return await self.call("${rpc.path}", req, ${outputName}, headers=headers)`);
      }
    }
    client.push('');
