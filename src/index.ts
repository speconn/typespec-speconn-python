...
function emitPython(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    client.push("from speconn import SpeconnClient, CallOptions");
    ...
    for (const rpc of svc.rpcs) {
      ...
      const outputName = rpc.outputType?.name || "dict";
      if (rpc.isStream) {
        client.push(`    async def ${rpc.name}(self, req: ${inputName}, *options: CallOptions) -> list[${outputName}]:`);
        client.push(`        return await self.stream("${rpc.path}", req, ${outputName}, *options)`);
      } else {
        client.push(`    async def ${rpc.name}(self, req: ${inputName}, *options: CallOptions) -> ${outputName}:`);
        client.push(`        return await self.call("${rpc.path}", req, ${outputName}, *options)`);
      }
    }
    client.push('');
