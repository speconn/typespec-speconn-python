...
function emitPython(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    server.push("from typing import Any");
    server.push("from speconn import SpeconnASGIApplication, SpeconnError, Endpoint, MethodInfo");
    server.push(`from ${importBase} import *\n`);
    server.push(`class ${svc.serviceName}Methods:`);
    for (const rpc of svc.rpcs) {
      const inputName = rpc.inputType?.name || "dict";
      const outputName = rpc.outputType?.name || "dict";
      server.push(`    ${rpc.name.toUpperCase()} = MethodInfo(`);
      server.push(`        name="${rpc.originalName}",`);
      server.push(`        service_name="${svc.serviceFQN}",`);
      server.push(`        input=${inputName},`);
      server.push(`        output=${outputName},`);
      server.push(`        stream_type="${rpc.isStream ? "server_stream" : "unary"}",`);
      server.push(`    )`);
    }
    server.push('');
    server.push(`class ${svc.serviceName}ASGI(SpeconnASGIApplication):`);
    server.push(`    def __init__(self, service: Any):`);
    server.push(`        super().__init__(`);
    server.push(`            service=service,`);
    server.push(`            endpoints={`);
    for (const rpc of svc.rpcs) {
      const epFn = rpc.isStream ? "server_stream" : "unary";
      server.push(`                "${rpc.path}": Endpoint.${epFn}(`);
      server.push(`                    method=${svc.serviceName}Methods.${rpc.name.toUpperCase()},`);
      server.push(`                    function=service.${rpc.name},`);
      server.push(`                ),`);
    }
    server.push(`            },`);
    server.push(`        )\n`);
    server.push(`    @property`);
    server.push(`    def path(self) -> str:`);
    server.push(`        return "/${svc.serviceFQN}"\n`);
    ...
    for (const rpc of svc.rpcs) {
      ...
      if (rpc.isStream) {
        ...
        client.push(`        return await self._call_stream("${rpc.path}", req, ${outputName})`);
      } else {
        client.push(`    async def ${rpc.name}(self, req: ${inputName}) -> ${outputName}:`);
        client.push(`        return await self._call_unary("${rpc.path}", req, ${outputName})`);
      }
    }
    client.push('');
